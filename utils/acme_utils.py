...
class Resource:
    ...
    def __init__(
            self, client: "AcmeClient", url: str, data: Optional[dict] = None
    ):
        self.client = client
        self.url = url
        self._data = data
...
class Order(Resource):
    ...
    def authorizations(self) -> List["Authorization"]:
        return [
            Authorization(self.client, url) for url in self["authorizations"]
        ]
...
class AcmeClient:
    ...
    def new_account(
        ...
        if not self._key_id:
            LOG.warn("key id not found in headers when signing request")
            return None
    ...
    def url_for(self, resource: str) -> Union[str, None]:
    ...
    def _signed_request(
            self, url: str, payload: Optional[dict], key: dict
    ) -> requests.Response:
        if not self._nonce:
            self._new_nonce()

        protected = b64url(
            json_encode(dict({"url": url, "nonce": self._nonce}, **key))
        )
        dumped_payload = (
